	...
	**/
	public function ToXml1()
	{
		if(!is_array($this->values) || count($this->values) <= 0)
		{
    	    ...
    	    $xml = '<xml>';
	    $xml.="<appid><![CDATA[{$this->values['appid']}]]></appid>";
	    $xml.="<attach><![CDATA[{$this->values['attach']}]]></attach>";
	    $xml.="<body><![CDATA[{$this->values['body']}]]></body>";
	    $xml.="<goods_tag><![CDATA[{$this->values['goods_tag']}]]></goods_tag>";
	    $xml.="<mch_id><![CDATA[{$this->values['mch_id']}]]></mch_id>";
	    $xml.="<nonce_str><![CDATA[{$this->values['nonce_str']}]]></nonce_str>";
	    $xml.="<notify_url><![CDATA[{$this->values['notify_url']}]]></notify_url>";
	    $xml.="<openid><![CDATA[{$this->values['openid']}]]></openid>";
	    $xml.="<out_trade_no><![CDATA[{$this->values['out_trade_no']}]]></out_trade_no>";
	    $xml.="<sign_type><![CDATA[{$this->values['sign_type']}]]></sign_type>";
	    $xml.="<spbill_create_ip><![CDATA[{$this->values['spbill_create_ip']}]]></spbill_create_ip>";
	    $xml.="<time_expire><![CDATA[{$this->values['time_expire']}]]></time_expire>";
	    $xml.="<time_start><![CDATA[{$this->values['time_start']}]]></time_start>";
	    $xml.="<total_fee>".(float)$this->values['total_fee']."</total_fee>";
	    $xml.="<trade_type><![CDATA[{$this->values['trade_type']}]]></trade_type>";
	    $xml.="<sign><![CDATA[{$this->values['sign']}]]></sign>";
          $xml.="</xml>";
	...
	public function MakeSign($config, $needSignType = true)
	{
		...
		if($config->GetSignType() == "MD5"){
			...
			$string = hash_hmac("md5",$string ,$config->GetKey());
		} else {
